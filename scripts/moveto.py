...
if __name__ == "__main__":
	...
	rospy.Subscriber("/drone/gt_pose", Pose , odom_callback)
	...
	while not rospy.is_shutdown():
		...
		#print(target, c_pose, heading_angle)
		...
		if state == 1:
			...
			vel_msg.linear.x = min(distance*0.5,20)
			
		elif state == 2:
			vel_msg.linear.z = -2
			vel_msg.angular.z = 0
			if abs(get_rotation_prop(current_angle, target[2], 0.2)) > 0.02:
				state = 1
				continue
			print(c_pose[2])
			if c_pose[2] < 0.2:
